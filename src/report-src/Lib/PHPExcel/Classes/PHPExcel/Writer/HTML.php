...
class PHPExcel_Writer_HTML extends PHPExcel_Writer_Abstract implements PHPExcel_Writer_IWriter
{
     ...
     * @throws PHPExcel_Writer_Exception
     */
    public function generateSheetData($sheetData = null, $isConfig = true)
    {
        // PHPExcel object known?
        if (is_null($this->phpExcel)) {
            throw new PHPExcel_Writer_Exception('Internal PHPExcel object not set to an instance of an object.');
        ...
        foreach ($sheets as $sheet) {
            ...
            while ($row++ < $rowMax) {
                ...
                if (!isset($this->isSpannedRow[$sheet->getParent()->getIndex($sheet)][$row])) {
                    ...
                    if(!$isConfig){
                        $html .= $this->generateRowWithDataInput($sheet, $rowData, $row - 1, $cellType, $sheetData);
                    } else{
                        $html .= $this->generateRowConfig($sheet, $rowData, $row - 1, $cellType, $sheetData);
                    }
                }
...
//        $css = $this->mapBorderStyle($pStyle->getBorderStyle()) . ' #' . $pStyle->getColor()->getRGB();
     ...
     * @throws    PHPExcel_Writer_Exception
     */
    private function generateRowWithDataInput(PHPExcel_Worksheet $pSheet, $pValues = null, $pRow = 0, $cellType = 'td', $sheetData = null)
    {
        if (is_array($pValues)) {
            // Construct HTML
            $html = '';
            ...
            foreach ($pValues as $cellAddress) {
                ...
                if ($writeCell) {
                    ...
                    // Cell data
                    if(isset($sheetData[$cellAddress]) && $sheetData[$cellAddress]["type"] == "1") {
                        $html .= $cellData 
                                . '<select name="'.$cellAddress. '.type" >'
                                . '<option value="0"></option>'
                                . '<option value="1"' . ((isset($sheetData[$cellAddress]) && $sheetData[$cellAddress]["type"] == "1") ? "selected" : "") . '>Yes</option>'
                                . ' </select>';
                        
                        
                        $html .=  '<br /><input type="text" placeholder="table name" name="'.$cellAddress. '.mapping_table" value="'. $sheetData[$cellAddress]["mapping_table"]  .'" />';
                        $html .=  '<br /><input type="text" placeholder="column name" name="'.$cellAddress. '.mapping_column" value="'. $sheetData[$cellAddress]["mapping_column"]  .'" />';
                        $html .=  '<br /><input type="text" placeholder="table name" name="'.$cellAddress. '.validate" value="'. $sheetData[$cellAddress]["validate"]  .'" />';
                    }
                    else {
                        $html .= $cellData;
                    }
    ...
    private function generateRowConfig(PHPExcel_Worksheet $pSheet, $pValues = null, $pRow = 0, $cellType = 'td', $sheetData = null)
    {
            ...
            foreach ($pValues as $cellAddress) {
                ...
                if ($writeCell) {
                    ...
                    if($cellAddress){
                        if( $cellData == ""){
                            $html .= $cellData 
                                    . '<select  name="' . $sheetIndex . '['.$cellAddress. '][type]" class="index" >'
                                    . '<option value="0"></option>'
                                    . '<option value="1"' . ((isset($sheetData[$cellAddress]) && $sheetData[$cellAddress]["type"] == "1") ? "selected" : "") . '>Yes</option>'
                                    . ' </select>';
                            $html .=  
                                    '<select  name="' . $sheetIndex . '['.$cellAddress. '][table_index]" style="display:none;">'
                                    . '<option value="0"' . ((isset($sheetData[$cellAddress]) && $sheetData[$cellAddress]["table_index"] == "0") ? "selected" : "") . '>0</option>'
                                    . '<option value="1"' . ((isset($sheetData[$cellAddress]) && $sheetData[$cellAddress]["table_index"] == "1") ? "selected" : "") . '>1</option>'
                                    . '<option value="2"' . ((isset($sheetData[$cellAddress]) && $sheetData[$cellAddress]["table_index"] == "2") ? "selected" : "") . '>2</option>'
                                    . '<option value="3"' . ((isset($sheetData[$cellAddress]) && $sheetData[$cellAddress]["table_index"] == "3") ? "selected" : "") . '>3</option>'
                                    . '<option value="4"' . ((isset($sheetData[$cellAddress]) && $sheetData[$cellAddress]["table_index"] == "4") ? "selected" : "") . '>4</option>'
                                    . ' </select>';
                            
                            $html .=  
                                    '<select  name="' . $sheetIndex . '['.$cellAddress. '][khoi_id]" style="display:none;">'
//                                    . '<option value="0"' . ((isset($sheetData[$cellAddress]) && $sheetData[$cellAddress]["khoi_id"] == "0") ? "selected" : "") . '>0</option>'
                                    . '<option value="1"' . ((isset($sheetData[$cellAddress]) && $sheetData[$cellAddress]["khoi_id"] == "1") ? "selected" : "") . '>Lớp 1</option>'
                                    . '<option value="2"' . ((isset($sheetData[$cellAddress]) && $sheetData[$cellAddress]["khoi_id"] == "2") ? "selected" : "") . '>Lớp 2</option>'
                                    . '<option value="3"' . ((isset($sheetData[$cellAddress]) && $sheetData[$cellAddress]["khoi_id"] == "3") ? "selected" : "") . '>Lớp 3</option>'
                                    . '<option value="4"' . ((isset($sheetData[$cellAddress]) && $sheetData[$cellAddress]["khoi_id"] == "4") ? "selected" : "") . '>Lớp 4</option>'
                                    . '<option value="5"' . ((isset($sheetData[$cellAddress]) && $sheetData[$cellAddress]["khoi_id"] == "5") ? "selected" : "") . '>Lớp 5</option>'
                                    . '<option value="7"' . ((isset($sheetData[$cellAddress]) && $sheetData[$cellAddress]["khoi_id"] == "7") ? "selected" : "") . '>Mẫu Giáo</option>'
                                    . '<option value="8"' . ((isset($sheetData[$cellAddress]) && $sheetData[$cellAddress]["khoi_id"] == "8") ? "selected" : "") . '>Nhà Trẻ</option>'
                                    . ' </select>';


                            $html .=  '<br /><input type="text" style="display:none;" placeholder="table name"  name="' . $sheetIndex . '['.$cellAddress. '][mapping_table]" value="'. (isset($sheetData[$cellAddress]["mapping_table"]) ? $sheetData[$cellAddress]["mapping_table"] : "")  .'" />';
                            $html .=  '<br /><input type="text" style="display:none;" placeholder="column name" name="' . $sheetIndex . '['.$cellAddress. '][mapping_column]" value="'. (isset($sheetData[$cellAddress]["mapping_column"]) ? $sheetData[$cellAddress]["mapping_column"] : "")  .'" />';
                            $html .=  '<br /><input type="text" style="display:none;" placeholder="validate" name="' . $sheetIndex . '['.$cellAddress. '][validate]" value="'. (isset($sheetData[$cellAddress]["validate"]) ? $sheetData[$cellAddress]["validate"] : "")  .'" />';
                        } else {
                            $html .= $cellData;
                        }
                    }
